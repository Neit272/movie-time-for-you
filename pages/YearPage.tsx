...
import { getContentByYear, getCountries, getCategories } from '../services/api';
...
export const YearPage = () => {
    ...
    const [showFilters, setShowFilters] = useSessionStorage(`year_${year}_show_filters`, false);
    
    const [categories, setCategories] = useState<Category[]>([]);
    const [countries, setCountries] = useState<Country[]>([]);
    const [category, setCategory] = useSessionStorage(`year_${year}_category`, '');
    const [country, setCountry] = useSessionStorage(`year_${year}_country`, '');
    ...
    const loadData = async (pageNum: number, isNew: boolean) => {
        ...
        const newItems = await getContentByYear(year, pageNum, {
            category,
            country
        });
    ...
    const activeFiltersCount = [category, country].filter(Boolean).length;
